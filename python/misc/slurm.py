			...
			for layers in [1, 2]:
				for dropout in [0, 0.5]:
					for bidirectional in [False, True]:
						...
						name = f'{net}-{hidden}-{layers}-{dropout}-{int(bidirectional)}-{embedding}'
						formatted = content.format(
							name,
							name,
							workers,
							time,
							f'-o {output} -net {net} -hidden {hidden} -layers {layers} -dropout {dropout} {biflag} -embedding {embedding} -workers {workers}'
						)
